...
export default function SelectedCoffee({ product }: { product: Product }) {
  ...
  const totalOrderValue = totalItemsValue + 3.5;
  ...
  return (
    <div className="w-full flex flex-col bg-card p-5 md:p-10 rounded-tr-3xl rounded-bl-3xl rounded-md">
      {cart.map((product, index) => (
        <div
          key={index}
          className="flex flex-row border-b-2 pb-6 pt-4 justify-between"
        >
          <div className="flex gap-5">
            <Image
              src={product.image}
              alt={"Café selecionado"}
              width={64}
              height={64}
              priority
            />
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-row gap-2">
                <div className="flex gap-2 justify-center items-center p-1 rounded-md bg-button text-text">
                  ...
                  <p className="text-title text-base leading-6">
                    {product.quantity}
                  </p>
                  <div className="bg-button p-0.5 cursor-pointer rounded-md hover:bg-hover">
                    <Plus
                ...
                <button
                  data-cy="remover-btn"
                  type="submit"
                  className="flex flex-row items-center gap-1 px-2 bg-button rounded-md hover:bg-hover"
          ...
          <p className="text-text font-bold">
            R$ <span>{product.value.toFixed(2)}</span>
          </p>
        </div>
      ))}
      <div className="flex flex-col gap-3 mt-3">
        <div className="flex flex-row justify-between">
          ...
          <p className="text-base text-text">
            R$ <span>{totalItemsValue.toFixed(2)}</span>
          </p>
        </div>
        <div className="flex flex-row justify-between">
        ...
        <div className="flex flex-row justify-between">
          ...
          <p className="text-xl font-bold text-subtitle">
            R$ <span>{totalOrderValue.toFixed(2)}</span>
          </p>
        </div>
        <Link
          href="/success"
          type="submit"
          data-cy="confirmar-btn"
          className="flex items-center justify-center bg-yellow hover:bg-yellow-dark p-3 rounded-md"
        >
